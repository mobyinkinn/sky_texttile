...
export default function Departments() {
  ...
  const handleToggleBlock = async (id, isBlocked) => {
    try {
      const endpoint = isBlocked
        ? `https://skytextilesbackend.pmcommu.in/api/v1/event/unblock-event/${id}`
        : `https://skytextilesbackend.pmcommu.in/api/v1/event/block-event/${id}`;
  ...
  const handleDeleteBlog = async (id) => {
    ...
    try {
      const response = await axios.get(
        `https://skytextilesbackend.pmcommu.in/api/v1/event/delete/${id}`
      );
  ...
  const handleUpdateBlog = async () => {
    ...
    try {
      const response = await axios.post(
        `https://skytextilesbackend.pmcommu.in/api/v1/event/update/${currentBlog._id}`,
        formData
      );
